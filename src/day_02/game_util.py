...
def parseDraw(draw):
    ...
    """
    draw = draw.split(',')
    return [parseMarble(marble) for marble in draw]

def parseDraws(draws):
    ...
    """
    draws = draws.split(';')
    return [parseDraw(draw) for draw in draws]
